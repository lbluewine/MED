...
/**
 * Telefone como as fontes escrevem: "(48) 3445-8730" ou "3445-8730". O DDD do
 * município completa o que falta na hora de mostrar.
 */
export const Telefone = z.string().regex(/^(\(\d{2}\) )?\d{4,5}-\d{4}$/);
...
export const ItemRemume = z.object({
  ...
  /**
   * Nulo quando a lista da cidade não diz. Não é raro: a REMUME de Campo
   * Grande publica o local de retirada de cada item e não o componente, e
   * deduzir o componente pela RENAME seria afirmar o que a fonte não afirma.
   *
   * A tela cai no que a fonte de fato dá — onde retirar —, que é o que a
   * pessoa precisa saber. Sem isto, a frase "quem entrega" some, em vez de
   * aparecer errada.
   */
  componente: Componente.nullable(),
  /**
  ...
  /**
   * Vazio quando a lista da cidade não diz o que levar. A tela então mostra só
   * as exigências gerais do município, e se nem essas houver, diz que não sabe
   * e manda perguntar na unidade — nunca inventa um documento.
   */
  exige: z.array(Exigencia),
...
// ---------------------------------------------------------------------------
// data/municipios/<id>/unidades-cnes.json
// ---------------------------------------------------------------------------

/**
 * Coordenada como o próprio estabelecimento declarou ao CNES.
 *
 * Terceiro tipo de coordenada do projeto, e separado pelo mesmo motivo dos
 * outros dois: `Geo` é o pino que uma pessoa abriu no mapa e confirmou que cai
 * na porta, `GeoDoCadastro` é o endereço casado com o cadastro do IBGE, e este
 * é o ponto que a unidade informou no cadastro federal. Erra pouco, mas erra
 * sozinho, e o site não pode dizer "conferimos" sobre o que ninguém conferiu.
 */
export const GeoDoCnes = z.object({
  lat: z.number().min(-34).max(6),
  lng: z.number().min(-74).max(-33),
  fonte: z.string().min(1),
  obtido_em: DataISO,
...
/**
 * O CNES tem 39 tipos de estabelecimento, do consultório isolado ao
 * laboratório de saúde pública. A tela não precisa dessa granularidade, e
 * jogar tudo numa lista só afogaria a UBS do bairro entre laboratórios.
 *
 * `outro` não é lixo: é tipo do CNES que ainda não foi classificado aqui. O
 * registro continua com `tipo_cnes` original, então nada se perde.
 */
export const CategoriaCnes = z.enum([
  /** Unidade Básica de Saúde, centro de saúde, posto. A porta de entrada. */
  "ubs",
  /** Centro de Atenção Psicossocial. Atende só quem é acompanhado no serviço. */
  "caps",
  /** Farmácia — inclui a do componente especializado e a Farmácia Popular. */
  "farmacia",
  /** UPA e pronto atendimento. */
  "pronto_atendimento",
  "hospital",
  /** Ambulatório, policlínica, clínica especializada, CEO. */
  "especializado",
  /** Laboratório, imagem, apoio ao diagnóstico. */
  "apoio_diagnostico",
  /** Secretaria, vigilância, central de regulação. Não atende ao público. */
  "gestao",
  "outro",
]);

/**
 * Estabelecimento de saúde como o cadastro federal registra.
 *
 * **Não é `Unidade`, e não pode virar uma.** `Unidade` responde "onde retirar
 * este medicamento": tem `dispensa`, `entrega_descricao` e horário conferido,
 * que são decisão da prefeitura e o CNES não sabe. Este aqui responde outra
 * pergunta, mais simples e nacional: "que unidades de saúde existem na minha
 * cidade, onde ficam e qual o telefone".
 *
 * Misturar os dois faria o site dizer que uma UBS entrega um medicamento
 * porque ela existe — que é exatamente a invenção que a regra 1 proíbe.
 */
export const UnidadeCnes = z.object({
  /** O código CNES, que é a chave estável do estabelecimento no país. */
  cnes: z.string().regex(/^\d{1,7}$/),
  /** O nome de porta. Nulo no cadastro, vale a razão social. */
  nome: z.string().min(1),
  categoria: CategoriaCnes,
  /** A descrição do tipo como o CNES escreve, sem tradução. */
  tipo_cnes: z.string().min(1),
  codigo_tipo_cnes: z.number().int().nonnegative(),
  /**
   * Se o estabelecimento faz atendimento ambulatorial pelo SUS. A lista da
   * tela mostra só quem faz — mandar alguém a uma clínica particular seria
   * pior que não mostrar nada.
   */
  atende_sus: z.boolean(),
  endereco: z.object({
    logradouro: z.string().min(1).nullable(),
    numero: z.string().min(1).nullable(),
    bairro: z.string().min(1).nullable(),
    cep: z.string().regex(/^\d{5}-\d{3}$/).nullable(),
  }),
  /**
   * Só os números que o cadastro traz em formato reconhecível. O que não dá
   * para ler com certeza fica em `observacoes`, escrito como veio: telefone
   * errado faz a pessoa ligar para a casa de um estranho.
   */
  telefones: z.array(Telefone),
  /**
   * O turno declarado, na palavra do CNES ("ATENDIMENTOS NOS TURNOS DA MANHA
   * E A TARDE"). Não é horário: o CNES não publica hora de abrir e fechar, e
   * o site não vai inventar uma.
   */
  turno: z.string().min(1).nullable(),
  geo: GeoDoCnes.nullable(),
  /** Quando o CNES registrou a última atualização deste estabelecimento. */
  atualizado_no_cnes_em: DataISO.nullable(),
  observacoes: z.string().min(1).nullable(),
});

export const UnidadesCnes = z.object({
  municipio_id: Slug,
  unidades: z.array(UnidadeCnes),
  proveniencia: Proveniencia,
}).refine(
  (u) => new Set(u.unidades.map((x) => x.cnes)).size === u.unidades.length,
  "há estabelecimentos com o mesmo código CNES",
);

// ---------------------------------------------------------------------------
// data/fontes/estado.json
// ---------------------------------------------------------------------------

/**
 * O que aconteceu com uma fonte na última conferência.
 *
 * `sem_verificacao` não é falha: é fonte sem URL estável, que só uma pessoa
 * atualiza. Dizer isso na cara é melhor que deixar parecer que o robô cuida.
 */
export const SituacaoDaFonte = z.enum([
  "igual",
  "mudou",
  "fora_do_ar",
  "sem_verificacao",
...
export const FonteConferida = z.object({
  id: z.string().min(1),
  descricao: z.string().min(1),
  tipo: z.enum(["arquivo", "extraida", "sem_verificacao"]),
  url: z.string().url().nullable(),
  arquivo: z.string().min(1),
  situacao: SituacaoDaFonte,
  /**
   * Quando o robô conseguiu olhar a fonte pela última vez. Nulo quando ela
   * nunca respondeu: carimbar hoje numa fonte que não respondeu seria mentir
   * exatamente no campo que existe para dar confiança.
   */
  conferida_em: DataISO.nullable(),
  /** Quando o documento mudou pela última vez. Nulo se nunca mudou aqui. */
  mudou_em: DataISO.nullable(),
});

/**
 * O estado da manutenção do site, publicado.
 *
 * Gerado por `scripts/verifica_fontes.py`, nunca escrito à mão. É a única
 * parte de `data/` que fala do site em vez de falar de saúde — e é pública de
 * propósito: um site que mostra a própria manutenção é mais confiável que um
 * que a esconde, e é assim que um estranho descobre que um link morreu.
 */
export const EstadoDasFontes = z.object({
  conferido_em: DataISO,
  fontes: z.array(FonteConferida).min(1),
});

// ---------------------------------------------------------------------------
// data/fontes/prefeituras.json
// ---------------------------------------------------------------------------

/**
 * Onde a REMUME de um município está, e se continua lá.
 *
 * `ancora` diz o que dá para fazer naquele site, não o que se gostaria:
 * `listagem_js` é registro honesto de "a página só monta no navegador", que é
 * o caso de Criciúma. Link direto é o degrau mais frágil — caminho de PDF não
 * sobrevive a troca de sistema —, e por isso mesmo é conferido toda semana.
 */
export const RemumeNoSite = z.object({
  ancora: z.enum(["direta", "listagem", "listagem_js"]),
  url_listagem: z.string().url().nullable(),
  /** Regex que casa com o texto ou o endereço do link certo na listagem. */
  regra: z.string().min(1).nullable(),
  url_documento: z.string().url().nullable(),
  observacao: z.string().min(1).nullish(),
  situacao: z.enum([
    /** O documento respondeu e é documento mesmo. */
    "no_lugar",
    /** O endereço respondeu, mas não devolveu documento: mudou de caminho. */
    "mudou_de_lugar",
    "fora_do_ar",
    /** A listagem respondeu e nenhum link casou com a regra. */
    "nao_achado",
    /** Só há ato no diário oficial, não o documento no site da prefeitura. */
    "so_no_diario",
    /** Acabaram os caminhos automáticos. Resta pedir pela LAI. */
    "sem_pista",
  ]).nullish(),
  achado_por: z.enum(["direta", "listagem", "querido_diario"]).nullish(),
  detalhe: z.string().min(1).nullish(),
  url_diario: z.string().url().nullish(),
  sha256: z.string().regex(/^[0-9a-f]{64}$/).nullish(),
  conferido_em: DataISO.nullish(),
});

export const PrefeituraNoCatalogo = z.object({
  slug: Slug,
  nome: z.string().min(1),
  uf: z.string().length(2),
  populacao: z.number().int().nonnegative(),
  dominio: z.string().min(1).nullable(),
  url: z.string().url().nullable(),
  titulo_visto: z.string().min(1).nullable(),
  /**
   * Como se sabe que aquele endereço é mesmo o da prefeitura. `titulo` é o
   * título da página dizendo "prefeitura" e o nome da cidade; `manual` é
   * alguém que conferiu. O que uma pessoa escreveu nunca é sobrescrito.
   */
  confirmacao: z.enum(["titulo", "responde", "manual", "nenhuma"]),
  verificado_em: DataISO,
  remume: RemumeNoSite.nullable(),
});

export const CatalogoPrefeituras = z.object({
  prefeituras: z.array(PrefeituraNoCatalogo),
});
...
export type Telefone = z.infer<typeof Telefone>;
export type SituacaoDaFonte = z.infer<typeof SituacaoDaFonte>;
export type RemumeNoSite = z.infer<typeof RemumeNoSite>;
export type PrefeituraNoCatalogo = z.infer<typeof PrefeituraNoCatalogo>;
export type CatalogoPrefeituras = z.infer<typeof CatalogoPrefeituras>;
export type FonteConferida = z.infer<typeof FonteConferida>;
export type EstadoDasFontes = z.infer<typeof EstadoDasFontes>;
export type GeoDoCnes = z.infer<typeof GeoDoCnes>;
export type CategoriaCnes = z.infer<typeof CategoriaCnes>;
export type UnidadeCnes = z.infer<typeof UnidadeCnes>;
export type UnidadesCnes = z.infer<typeof UnidadesCnes>;
