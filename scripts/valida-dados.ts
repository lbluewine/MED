...
for (const id of municipios) {
  try {
    ...
    // O cadastro federal, quando existe para esta cidade. Não substitui
    // unidades.json: responde "que unidades existem", não "onde retirar".
    const cnes = carregaUnidadesCnes(id);
    if (cnes === null) {
      aviso(
        `${id}: sem unidades-cnes.json. Rode scripts/baixa_cnes.py e ` +
          `scripts/extrai_unidades_cnes.py para publicar a rede de saúde.`,
      );
    } else {
      const sus = cnes.unidades.filter((u) => u.atende_sus);
      const semEndereco = sus.filter((u) => u.endereco.logradouro === null).length;
      const semMapa = sus.filter((u) => u.geo === null).length;
      console.log(
        `${id} no CNES: ${cnes.unidades.length} estabelecimentos, ` +
          `${sus.length} atendem pelo SUS (${semMapa} fora do mapa)`,
      );
      if (semEndereco > 0) {
        aviso(`${id}: ${semEndereco} estabelecimento(s) do CNES sem logradouro.`);
      }
      const naoCatalogados = new Set(
        cnes.unidades
          .filter((u) => u.tipo_cnes.includes("NÃO CATALOGADO"))
          .map((u) => u.codigo_tipo_cnes),
      );
      if (naoCatalogados.size > 0) {
        aviso(
          `${id}: tipo(s) do CNES fora da tabela local: ` +
            `${[...naoCatalogados].join(", ")}. Rebaixe com --tipos.`,
        );
      }
    }
