 ...
 */
export default function MedicamentosDoPrograma({ municipioId }: { municipioId?: string }) {
  ...
  // `resolveMunicipio` porque cidade sem lista própria não tem municipio.json.
  ...
  */
  ...
  if (municipioNav && resolvido?.temRemume) {
  ...
  for (const item of itensSemListaMunicipal()) {
    for (const a of item.apresentacoes) {
      paginaDoItem.set(
        a.texto,
        municipioNav
          ? `/${municipioNav}/farmacia-popular/${item.slug}`
          : `/farmacia-popular/${item.slug}`,
      );
    }
  }
  ...
  return (
    <Pagina municipioId={municipioNav} atual="farmacia-popular">
      ...
      <p className="mt-6 max-w-[68ch] leading-normal text-texto-suave">
        ...
        ou <a href={municipioNav ? `/${municipioNav}/farmacia-popular` : "/farmacia-popular"}>entenda como o programa funciona</a>.
      </p>
