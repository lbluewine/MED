...
import { carregaFarmaciasPopulares, resolveMunicipio } from "@/lib/dados";
...
export default function FarmaciasDoPrograma({ municipioId }: { municipioId?: string }) {
  const municipioNav = municipioId;
  const elenco = elencoFarmaciaPopular();
  // `resolveMunicipio` e não `carregaMunicipio`: cidade sem lista própria não
  // tem municipio.json, e ler direto quebraria a página em vez de avisar.
  ...
  const migalha = [
    { texto: "Início", href: municipioNav ? `/${municipioNav}` : "/" },
    {
      ...
      href: municipioNav ? `/${municipioNav}/farmacia-popular` : "/farmacia-popular",
    },
    { texto: "Onde tem" },
  ];
