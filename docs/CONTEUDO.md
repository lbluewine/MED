...
O relato vai para as issues do repositório, com título e corpo já preenchidos
(`lib/relato.ts`). Três tipos, com etiqueta própria: `dado` para erro numa tela,
`fonte` para documento que saiu do lugar, `remume` para dizer onde a lista de um
município está publicada.

Site estático não recebe formulário, e nenhuma porta de escrita pública foi
aberta de propósito: o que chega vira fila para alguém triar, nunca publicação.

**O canal exige conta no GitHub, e isso exclui parte do público deste site** —
pessoa idosa, pouca familiaridade digital. A decisão foi consciente: é o único
canal com custo zero e sem porta de escrita para proteger. Enquanto for assim,
a tela **diz** que precisa de conta, em vez de levar a pessoa a um beco. Se um
dia houver um canal aberto a qualquer um, ele entra ao lado deste, não no lugar.
